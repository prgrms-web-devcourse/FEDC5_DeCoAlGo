function solution(numbers, hand) {
  ...
  numbers.forEach((val) => {
    if (val == 1 || val == 4 || val == 7 || val == "*") {
      ...
      const left_dis =
        Math.abs(keypad[val][0] - left_loc[0]) +
        Math.abs(keypad[val][1] - left_loc[1]);
      const right_dis =
        Math.abs(keypad[val][0] - right_loc[0]) +
        Math.abs(keypad[val][1] - right_loc[1]);
